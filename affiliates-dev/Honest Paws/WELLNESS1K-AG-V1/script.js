    ...
    function handleFormSubmit(form, input) {
      ...
      if (!errors) {
        ...
        createOrder(dataSend, function(order) {
            ...
            $('.type').html(product.type);
            $('.subtotal').html(product.subtotal.toFixed(2));
            $('.subtotal_total').html(parseFloat(order.orderTotal).toFixed(2));
            $('.discounts').html(product.discounts);
            $('.discounts').html(product.discounts);
            ...
            if (dataSend.shipping_address2) {
              $('.shipping-suite').html(" Suite " + dataSend.shipping_address2);
            }
            ...
            $('.billing-name').html(dataSend.first_name);
            $('.billing-lastname').html(dataSend.last_name);
            $('.billing-address').html(dataSend.billing_address1);
            if (dataSend.billing_address2) {
              $('.billing-suite').html(" Suite " + dataSend.billing_address2);
            }
            $('.billing-city').html(dataSend.billing_city);
            $('.billing-state').html(dataSend.billing_state);
            $('.billing-zipcode').html(dataSend.billing_zip);
            $('.billing-country').html(dataSend.billing_country);
            setTimeout(function(){
              $('#upsell_container').show();
              var upsell_product_id_initial = $('input[name=upsell_product_id]:checked').val();
              $('.upsell-original-price').html("$"+upsells[upsell_product_id_initial].original_price);
              $('.upsell-price').html("$"+upsells[upsell_product_id_initial].price);
              ...
              $('.buy-upsell').click(function(){
                ...
                createUpsell(dataSend2, function(upsell_created) {
                    $('.upsell_id').html("#"+upsell_created.order_id);
                    $('.upsell_item').show();
                    ...
                    $('.total').html(parseFloat(parseFloat(order.orderTotal) + parseFloat(upsell_created.orderTotal)).toFixed(2));
                    $('.upsell_image').attr('src', upsells[upsell_product_id].image);
                    $('.upsell_name').html(upsells[upsell_product_id].name);
                    dataLayer.push({
...
function calculateTaxes() {
  ...
  if (zipcode && country && state && shipping_id) {
    ...
    calculateTotal(dataSend, function(data) {
        if (data.data) {
          if (data.data.shipping || data.data.tax) {
          ...
          if (data.data.tax) {
            $('.taxes').html(data.data.tax.total.toFixed(2));
